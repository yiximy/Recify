...
- **数据持久化**：所有关联、金额、确认态原子写入本地 JSON，重启不丢失
...
> **首次 OCR 卡住？** PaddleOCR 首次使用需下载 PP-OCRv5 模型（约数十 MB），状态栏会提示「正在加载 OCR 模型」。模型缓存在本地，后续启动不再下载。
>
...
>
> **重命名关联后下次扫描会丢失关联吗？** 不会。重命名操作同步更新了 Store 内的 file_id 及所有交叉引用，重扫后关联关系完整保留。
