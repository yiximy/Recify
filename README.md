...
创造灵感：本人因出差频繁，发票与支付记录经常堆积如山，核对工作耗时费力。为此，开发了一个自动化辅助工具，让繁琐的核对变得轻松高效。
...
- **自动比对关联**：以可视化流程画布自动铺出金额匹配候选链（发票模块 → 匹配模块 → 支付模块，一个匹配模块可连多张同额支付）。双击模块可改绑文件/组合与金额容差，删除「匹配 → 支付」连线即取消该条匹配，「确定」按画布连线批量自动关联，自动关联结果以标记区分于手动关联
...
1. 回到「比对关联模式」
2. **自动比对（流程画布）**：
   - 点击「自动比对」打开画布，系统自动铺出金额匹配的候选链：每张发票（或组合）对应一个「匹配」模块，同额支付模块接在其输出端（一发票配多支付时自动聚合为一条匹配模块多条支付支线）
   - 双击发票/支付模块：从该类型文件与组合列表中多选绑定（组合整组参与，空壳组合自动过滤），可改名
   - 双击「匹配」模块：调整金额容差；删除「匹配 → 支付」连线可取消单条候选
   - 从左侧模块库可拖入模块手动搭建新链
   - 点「确定并执行」→ 逐条支线按金额容差校验后批量自动关联（容差不符的支线跳过并列出原因，自动关联已标记）
...
└── app\
    ...
    └── ui\
        ...
        │   ├── match_flow\             # 自动比对流程画布子包（model/items/canvas/config_dialog）
        │   ├── match_flow_dialog.py    # 自动比对流程画布对话框
        │   ├── amount_edit_cell.py     # 金额编辑单元格
        │   ├── confirm_checkbox.py     # 确认复选框（自绘矢量对号）
        │   ├── status_badge.py         # 关联状态徽标（未关联 / 手动关联 / 自动关联）
        │   ├── toggle_switch.py        # 通用滑动开关组件
